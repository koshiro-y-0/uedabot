...
### Jinja2テンプレート
- [ ] `templates/report.j2` を作成（通知メッセージのひな形）
- [ ] 植田総裁口調テンプレート文を全パターン定義する
  - [ ] 金利：変化なし
  - [ ] 金利：引上げ
  - [ ] 金利：引下げ
  - [ ] CPI：上昇
...
### レポート生成ロジック
- [ ] `src/generate_report.py` を作成
- [ ] `fetch_indicators.py` から取得したデータをテンプレートに埋め込む処理を実装
- [ ] 指標の条件分岐（金利変動・CPI変動）でテンプレートを切り替えるロジックを実装
- [ ] 生成されたレポートのフォーマット確認（手動テスト）

### 成果物
- [ ] `src/generate_report.py` を作成・PR提出
- [ ] `templates/report.j2` を作成・PR提出
